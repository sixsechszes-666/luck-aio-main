...
import { Search, Timer, CheckCircle2, ArrowRightLeft, ExternalLink, Loader2, ArrowUpDown } from "lucide-react";
import { Input } from "@/components/ui/input";
import { StatusBadge } from "@/components/StatusBadge";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { type RenewAccount, type RenewSummary } from "@/data/mockData";
import { apiGetRenew } from "@/services/api";
...
const emptySummary: RenewSummary = { totalAccounts: 0, successCount: 0, forwardSuccess: 0, reverseSuccess: 0 };
...
export const RenewTimersTab = () => {
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState<"ALL" | "SUCCESS" | "ERROR">("ALL");
  const [accounts, setAccounts] = useState<RenewAccount[]>([]);
  const [summary, setSummary] = useState<RenewSummary>(emptySummary);
  const [loading, setLoading] = useState(true);
  const [sortConfig, setSortConfig] = useState<{key: string, direction: 'asc'|'desc'} | null>(null);

  useEffect(() => {
    setLoading(true);
    apiGetRenew().then((res) => {
      if (res.data && res.summary) {
        setAccounts(res.data.map((r: any) => ({ udDir: r.UD_DIR, workerId: r.WORKER_ID, result: r.RESULT, forwardTransactionStatus: r.FORWARD_TRANSACTION_STATUS || "", reverseTransactionStatus: r.REVERSE_TRANSACTION_STATUS || "", link: r.LINK || "" })));
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bw-card rounded-sm p-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-5">
          <span className="bw-label text-sm">Renew Logs · {filtered.length}</span>
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center text-[#888] py-12 bw-label text-sm">No data</TableCell></TableRow>
              : filtered.map((a) => (
